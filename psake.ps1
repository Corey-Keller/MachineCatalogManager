...
Task PostDeploy -depends Deploy {
    ...
    If ($ENV:BHBuildSystem -eq 'AppVeyor')
    {
        ...
        cmd.exe /c "git config --global credential.helper store 2>&1"
        
        Add-Content "$env:USERPROFILE\.git-credentials" "https://$($env:access_token):x-oauth-basic@github.com`n"
        
        "git config --global user.email"
        cmd.exe /c "git config --global user.email ""$($ENV:BHProjectName)-$($ENV:BHBranchName)-$($ENV:BHBuildSystem)@cmkeller.com"" 2>&1"
        
        "git config --global user.name"
        cmd.exe /c "git config --global user.name ""AppVeyor"" 2>&1"
        
        "git config --global core.autocrlf true"
        cmd.exe /c "git config --global core.autocrlf true 2>&1"
    }
    
    "git checkout $ENV:BHBranchName"
    cmd.exe /c "git checkout $ENV:BHBranchName 2>&1"
    
    "git add -A"
    cmd.exe /c "git add -A 2>&1"
    
    "git commit -m"
    cmd.exe /c "git commit -m ""AppVeyor post-build commit[ci skip]"" 2>&1"
    
    "git status"
    cmd.exe /c "git status 2>&1"
    
    "git push origin $ENV:BHBranchName"
    cmd.exe /c "git push origin $ENV:BHBranchName 2>&1"
